...
$(function() {
  $('.portfolio-link').on('click', function(event) {
    ...
    if (window.forLanguage === true)  //english use
    {
      ...
      {
        description.html("This project is a basic graphics engine using OpenGL. It is a solol project made using C++ and OpenGL and GLSL. <br> This is a graphic engine project that I made using OpenGL for the first time. Includes simple, basic graphics engine functionality.  <br> - Keyboard and mouse Input <br> - Window creation and control <br> - Rendering 50 Objects <br> - Catmull - Rom Spline <br> - Back Face Curling <br> This graphics engine project encompasses very basic functionalities, but it's a project of great significance to me because it's the first project I've created on my own using OpenGL. Integrating the features I implemented one by one into a unified engine allowed me to enhance my skills in combining and integrating diverse pieces of code. Through this project, I was able to render what I desired using the OpenGL graphics API and create a complete engine, which brought me a sense of accomplishment and pride.");
        role.text("Solo Proejct");
        date.text("March 2020 - August 2020");
        category.text("Graphics Engine");
      }
    ...
    else  //korean use
    {
      ...
      {
        description.html("Pang Pang Dust 2D 플랫포머 슈팅 게임으로 미세먼지를 청소기로 흡수하고 초미세먼지를 공격해 정화하며 골인지점으로 달리는 게임입니다. 플레이어는 키보드로 이동하고 마우스를 이용해 슈팅 할 수 있으며 여러가지 아이템을 이용해 게임을 즐길 수 있습니다. <br> 이 프로젝트는 c++로 제작되었으며 상용 엔진이 아니라 교수님이 직접 제작한 엔진으로 구현되어졌습니다. 저는 팀의 프로듀서로써 멤버들을 총괄하고 게임의 체력과 미세먼지의 생성과 캐릭터의 움직임을 담당하였습니다. 뿐만 아니라 게임의 스코어 시스템과 맵 디자인을 담당했고 슈팅의 게이지를 제작하였습니다. <br> 직접적으로 게임을 처음 제작해본 프로젝트였기 때문에 부족한 점도 많았고 어려운 점도 많았지만 팀원들과 함께 화합하며 본격적으로 게임을 제작 할 때 어떤 능력들이 필요하고 것이 어떤 것인지에 대해 배워 갈 수 있었습니다. 부족한 점도 많았던 프로젝트였지만 게임 프로젝트에 대한 기초를 다질 수 있었던 프로젝트였습니다.");
        role.text("프로듀서");
      ...
      {
        description.html("이 프로젝트는 Lighting을 포함하는 그래픽 엔진 입니다. C++과 OpenGL, Im Gui 그리고 GLSL을 활용하여 제작한 1인 프로젝트 입니다. <br> 이 그래픽 엔진을 위해 오브젝트를 로딩하고 텍스처를 매핑하는 기본적인 기능을 하는 그래픽 엔진을 만들었으며 Phong Lighting, Phong Shading, Blinn shading이 가능한 Shader를 만들었습니다 <br> 뿐만 아니라 저는 최대 16개까지 늘어나는 조명을 추가하였고 사용자가 이 조명들의 개별 값을 설정할 수 있도록 하였는데 설정 가능 한 값으로는 아래 항목이 있습니다. <br> - Point Light, Directional Light, Spot Light 중에서 타입 고르기 <br> - Inner and outer angle for spot light <br> - Spot falloff <br>  또 한 설정 가능한 글로벌 값으로는 아래 항목이 있습니다 <br> - Ambient, diffuse and specular color <br> - attenuation coefficients <br> - atmospherc(fog) color <br> - Global ambient color <br> 또 한 텍스처를 매핑할 떄 material coefficients의 색상을 변경할 수 있으며 CPU에서의 계산과 GPU에서의 계산을 따로 설정 할 수 있도록 하였습니다. <br> 혼자서 만들어야 하는 그래픽 엔진이었기 때문에 많은 어려움이 있었지만 한단계 한단계 차근차근 문제를 해결해 나갔습니다.");
        role.text("1인 프로젝트");
