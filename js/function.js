...
$(function() {
  $('.portfolio-link').on('click', function(event) {
    ...
    else  //korean use
    {
      ...
      if(postModalId == "portfolioModal-1")
      {
        description.html("PooPooPiPe는 2D 퍼즐 게임으로 시작 점에서 끝 지점까지 파이프를 연결하여 주인공인 Poo가 이동할 수 있도록 하는 게임입니다. 플레이어는 마우스를 사용하여 파이프 퍼즐을 드래그 하여 움직일 수 있으며 클릭하여 파이프 퍼즐의 방향을 회전 할 수 있습니다. 파이프 퍼즐을 완성하면 Poo가 도착 장소로 이동하며 다음 스테이지로 넘어갑니다. 회전 횟수 제한 움직이는 파이프 퍼즐등의 요소를 통해 플레이어에게 더큰 즐거움을 줄 수 있도록 하였습니다. <br><br> 이 프로젝트는 C++로 제작되었으며 자체 제작한 엔진으로 구현되었습니다. 저는 팀의 프로듀서로써 프로젝트의 전체적인 과정을 총괄하고 게임 엔진을 구현하였을 뿐만 아니라 대부분의 게임 스테이지및 게임 요소들을 제작하였습니다. <br><br> 게임 엔진을 직접 만들 때 구체적인 정보가 없어서 어려움을 겪었지만 여러가지 정보를 찾아보고 게임 디자인을 위한 책을 읽으며 엔진을 만들 수 있었습니다. 게임을 빠르게 제작해야 했기 때문에 엔진을 제작 할 시간은 더더욱 부족했기 때문에 힘든 시간이었지만 직접 엔진을 제작함으로써 엔진에 대해 많이 배울 수 있었습니다. <br><br> 또한 프로젝트 진행 중 일을 반복적으로 제시간에 완성하지 못하는 팀원과 다른 팀원 사이의 불화가 있었지만 프로듀서로써 화합을 위한 미팅을 진행하고 또 직접 문제가 발생한 팀원에게 할당된 일을 도우면서 갈등을 해결 할 수 있었습니다. <br><br> 처음으로 직접 엔진부터 만들어 가는 프로젝트였기 때문에 만드는 동안 많은 어려움이 있었고 시간도 촉박했지만 완성된 게임은 우리 팀만의 확실한 색깔을 담고 있으며 게임의 시작부터 끝까지 모든 부분을 직접 만들어 볼 수 있어서 좋았습니다. 뿐 만 아니라 이 프로젝트를 하나의 게임으로써 좋게 마무리 할 수 있어서 뜻 깊은 프로젝트였습니다.");
      ...
      {
        description.html("Release From Perdition 3D 1인칭 슈팅 게임으로 슈팅과 특수 능력으로 수많은 몬스터들을 물리치고 실종된 팀원을 찾으러가는 여정을 담은 게임입니다. 플레이어는 키보드로 이동하고 마우스를 이용해 무기를 바꾸고 슈팅을 할 수 있으며 위치를 바꾸고 폭팔 능력이 있는 그림자를 스폰하는 특수 능력을 사용할 수 있습니다. <br><br> 이 프로젝트는 언리얼 엔진4와 c++을 이용하여 제작되었습니다. 저는 팀의 게임플레이 프로그래머로써 플레이어의 움직임과 몬스터너 스포너를 담당하여 몬스터의 스폰과 체력 그리고 스포너 이펙트를 구현하였습니다. 뿐만 아니라 몬스터를 제작하는 팀원과 함께 몬스터의 AI가 좀 더 똑똑하게 작동될 수 있도록 도왔습니다. <br><br> 언리얼 엔진을 처음 사용해보았기 때문에 언리얼 엔진에 대한 이해도가 낮아 많은 어려움이 있었지만 많은 자료를 찾아보며 언리얼 엔진에 빠르게 익숙해 지기 위해 노력했습니다. 뿐만 아니라 필요한 자료들이 많이 있지 않아서 자료를 직접 찾아보기 보다는 최대한 스스로 해결하기 위해 노력했습니다.  특히 각 몬스터들을 다른 사람들이 만들었기 때문에 이것들을 한번에 관리해야하는 스포너를 담당하는 저로써는 많은 어려움이 있었습니다. 따라서 스포너에서 자체적으로 필드에 살아있는 몬스터 수를 파악할 수 있도록 하였고 게임 디자이너들이 맵을 구성 할 때 쉽게 필요한 만큼 몬스터의 타입과 수 그리고 부활 횟수를 설정 할 수 있도록 하였습니다. <br><br> 인종, 나이, 전공이 모두 다른 팀원들과 함께 하면서 다양한 사람들과 함께 협업하며 게임을 만드는 과정을 배울 수 있었습니다. 팀원들과 매주 미팅을하며 서로 어려움이 있으면 바로 공유하며 문제를 해결해 나갔고 함께 아이디어를 공유하며 항상 게임을 어떻게 해야 더 좋은 방향으로 발전 시킬 수 있을지 고민하며 많은 것들을 배울 수 있었던 프로젝트였습니다. ");
      ...
      {
        description.html("Pang Pang Dust 2D 플랫포머 슈팅 게임으로 미세먼지를 청소기로 흡수하고 초미세먼지를 공격해 정화하며 골인지점으로 달리는 게임입니다. 플레이어는 키보드로 이동하고 마우스를 이용해 슈팅 할 수 있으며 여러가지 아이템을 이용해 게임을 즐길 수 있습니다. <br><br> 이 프로젝트는 c++로 제작되었으며 상용 엔진이 아니라 교수님이 직접 제작한 엔진으로 구현되어졌습니다. 저는 팀의 프로듀서로써 멤버들을 총괄하고 게임의 체력과 미세먼지의 생성과 캐릭터의 움직임을 담당하였습니다. 뿐만 아니라 게임의 스코어 시스템과 맵 디자인을 담당했고 슈팅의 게이지를 제작하였습니다. <br><br> 직접적으로 게임을 처음 제작해본 프로젝트였기 때문에 부족한 점도 많았고 어려운 점도 많았지만 팀원들과 함께 화합하며 본격적으로 게임을 제작 할 때 어떤 능력들이 필요하고 것이 어떤 것인지에 대해 배워 갈 수 있었습니다. 부족한 점도 많았던 프로젝트였지만 게임 프로젝트에 대한 기초를 다질 수 있었던 프로젝트였습니다.");
        role.text("프로듀서");
      ...
      {
        description.html("이 프로젝트는 아주 기본적인 게임 엔진의 구조를 구현한 프로젝트입니다. 엔진말고도 두개의 간단한 2D 게임이 포함되어져 있으며 한가지는 장애물을 피해 도착지점까지 달리는 플랫포머 게임이고 다른 한가지는 우주선을 이동하며 장애물을 파괴하는 슈팅게임입니다. <br><br> 이 프로젝트는 c++로 제작되어졌으며 게임 엔진을 만들기 위한 기본적인 이해를 돕고 만들어진 엔진을 통해 간단한 게임을 구현하는 프로젝트입니다. 1인 프로젝트였기 때문에 모든 부분은 직접 제작하였지만 간단한 게임을 구현 할 때 사용 된 그래픽 엔진은 교수님께 제공받았습니다. <br><br> 디버그를 출력하는 기능 부터, 엔진 루프, fps, 컴포넌트, 스테이지간의 연결등 기본적으로 게임 엔진에 포함되어지는 기능들을 직접구현하였습니다. 이렇게 만들어진 엔진을 이용하여 만든 간단한 게임에서는 캐릭터의 움직임과 간단한 AABB 그리고 슈팅 기능과 Parallax scrolling background과 Wrapping등 게임에서 자주 사용되는 기본적인 테크닉을 구현하였습니다. <br><br> 게임 엔진의 구조와 기본적인 기능을 본격적으로 배울 수 있었던 프로젝트 입니다. 게임 엔진에 대한 기본 지식 없이 프로젝트를 진행하다가 많이 힘들었던 경험이 있었기 때문에 그 프로젝트를 진행하기 전에 이 프로젝트를 진행했다면 좋았을 것같다는 생각이 들었습니다. 다시 게임 엔진에 대한 기초를 다질 수 있는 프로젝트였습니다.");
        role.text("1인 프로젝트");
      ...
      {
        description.html("이 프로젝트는 Lighting을 포함하는 그래픽 엔진 입니다. C++과 OpenGL, Im Gui 그리고 GLSL을 활용하여 제작한 1인 프로젝트 입니다. <br><br> 이 그래픽 엔진을 위해 오브젝트를 로딩하고 텍스처를 매핑하는 기본적인 기능을 하는 그래픽 엔진을 만들었으며 Phong Lighting, Phong Shading, Blinn shading이 가능한 Shader를 만들었습니다 <br><br> 뿐만 아니라 저는 최대 16개까지 늘어나는 조명을 추가하였고 사용자가 이 조명들의 개별 값을 설정할 수 있도록 하였는데 설정 가능 한 값으로는 아래 항목이 있습니다. <br> - Point Light, Directional Light, Spot Light 중에서 타입 고르기 <br> - Inner and outer angle for spot light <br> - Spot falloff <br><br>  또 한 설정 가능한 글로벌 값으로는 아래 항목이 있습니다 <br> - Ambient, diffuse and specular color <br> - attenuation coefficients <br> - atmospherc(fog) color <br> - Global ambient color <br><br> 또 한 텍스처를 매핑할 때 material coefficients의 색상을 변경할 수 있으며 CPU에서의 계산과 GPU에서의 계산을 따로 설정 할 수 있도록 하였습니다. <br><br> 혼자서 만들어야 하는 그래픽 엔진이었기 때문에 많은 어려움이 있었지만 한단계 한단계 차근차근 문제를 해결해 나갔습니다.");
        role.text("1인 프로젝트");
      ...
      {
        description.html("이 프로젝트는 Reflection과 Refraction을 포함하는 Environment Mapping을 구현한 그래픽 엔진 입니다. C++과 OpenGL, Im Gui 그리고 GLSL을 활용하여 제작한 1인 프로젝트 입니다. <br><br> 이 그래픽 엔진을 위해 오브젝트를 로딩하고 텍스처를 매핑하는 기본적인 기능을 하는 그래픽 엔진 기능을 포함하며  cube mapping을 이용한 sky box, FBO를 이용한 scene 렌더링, Environment Mapping이 가능한 Shader를 만들었습니다 <br><br> Environment Mapping은 아래의 항목을 포함합니다 <br> - Only reflection <br> - Only refraction <br> - Combination of both <br> 또한 Phong shading과 Environment mapping이 동시에 작동되도록 하였습니다. <br><br> 사용자는 오브젝트의 굴절률을 직접 세팅 할 수 있으며 material을 선택해 각기 다른 굴절률을 직접 확인 할 수 있습니다. <br><br> 먼저 만들어진 그래픽 엔진을 기반으로 Environment mapping을 구현한 그래픽 엔진입니다. 중간중간 엔진 자체를 뒤엎으면서 많은 수정을하기도 했고 한꺼번에 많은 걸 구현하기 위해 많은 시간을 쏟는 등 엔진을 만드는 과정이 순탄치 않았지만 본격적으로 그래픽 엔진에 대한 이해를 높이고 개발에 즐거움을 배울 수 있는 프로젝트였습니다.");
        role.text("1인 프로젝트");
      ...
      {
        description.html("이 프로젝트는 DiretX를 이용한 기본적인 그래픽 엔진입니다. C++과 DirectX 그리고 GLSL을 활용하여 제작한 1인 프로젝트 입니다. <br><br> 이 그래픽 엔진은 DiretX를 공부하고 활용하는 방법을 배우기 위해 시작된 프로젝트입니다. 간단하고 기초적인 그래픽 엔진의 기능을 포함하고 있습니다. <br> - 키보드 및 마우스 인풋 <br> - 윈도우 창 생성 및 제어 <br> - 간단한 도형을 생성 <br> - 모델 변환 (회전, 이동) <br> - Math Library 구현 <br><br> 이 그래픽 엔진은 매우 기초적인 기능을 포함하고 있지만 평소 자주 사용하던 OpenGL이 아니라 DirectX라는 새로운 그래픽 API를 배우기 위한 프로젝트였습니다. DirectX는 아예 처음이었고 그 동안 공부해 본 적이 없었기 때문에 시작 부터 많은 난관이있었지만 이를 통해 새로운 것에 도전하는 즐거움과 어떤 새로운 프로그램이나 API를 마주하더라도 배우고 발전 시킬 수 있을 것 같다는 용기를 얻을 수 있었습니다. ");
        role.text("1인 프로젝트");
      ...
      {
        description.html("Are You Ready는 Unity를 이용해 만든 안드로이드 AR 프로젝트입니다. 이 프로젝트는 3가지 종류의 시뮬레이션을 포함합니다. <br> - GPS를 사용한 볼링 시뮬레이션 <br> - 이미지 트래킹을 사용한 AR 명함 <br> - Photon Networking을 이용한 색깔 바꾸기 게임 <br><br> 이 프로젝트는 C#으로 제작되었으며 Unity 엔진과 AR Foundation 그리고 photon을 사용하여 구현하였습니다. 2인 팀프로젝트로 저는 사용자가 적절한 평면을 찾고 오브젝트를 배치하며 오브젝트의 크기와 위치를 변경할 수 있도록하였습니다. 또한 사용자의 위치를 받아와 위치에 맞는 적절한 오브젝트를 스폰 할 수 있도록 하였습니다. 뿐만 아니라 Photon을 이용하여 사용자간 오브젝트와 평면의 위치와 속성을 동기화 시켜 게임이 진행 되도록 하였습니다. <br><br> 처음으로 진행한 AR 프로젝트로 새로운 기술을 공부하고 또 활용하기 위해 진행한 프로젝트 입니다. 2명이서 진행한 프로젝트지만 다양한 기술들을 활용 할 수 있도록 단순한 AR 프로젝트를 넘어 Image tracking과 Photon등을 사용하여 여러가지 시뮬레이션을 제작 할 수 있었습니다. 처음 배우는 AR이라서 많은 문제가 있었고 심지어 자료 조차 많이 없어서 힘들었지만 팀원과 함께 문제를 해결해 나가려 노력하며 문제 해결 능력을 키울 수 있었습니다.");
        role.text("2인 프로젝트");
